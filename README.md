# Growth Daily — Build Your Own Growth Tracker

A personal, offline-capable dashboard for tracking whatever daily, weekly,
monthly, quarterly, and annual habits matter to you — with check-ins, 1–10
self-scoring, a streak counter, and a nightly journal. The first time you
open it, you'll set up your own tracker: pick which rhythms you want to
keep, then choose from suggested habits or add your own. Installs to your
phone's home screen like a native app. No account, no server, no ongoing
cost.
...
## First run — setting up your tracker

When you open the app for the first time, you'll land on a setup screen
instead of the dashboard:

1. **Pick your rhythms** — tap Daily, Weekly, Monthly, Quarterly, and/or
   Annual. You don't have to use all five; pick whichever cadences make
   sense for you.
2. **For each rhythm you picked**, choose from a list of suggested items
   (tap to add), or type your own into "Add your own item…" and hit Add.
3. Once you've added at least one item, **Start tracking** becomes active.
   Tap it and you're into the dashboard.

You can come back to this setup screen any time by tapping the pencil icon
in the dashboard's header — your existing check-ins and scores for items
you keep are preserved; only items you remove stop being tracked.
...
Your tracker setup (which rhythms and items you chose) and your check-ins,
scores, and journal entries are stored in your phone browser's **local
storage**, tied to the exact URL you installed from. This means:

- Clearing Safari/Chrome site data for that URL will erase everything,
  including your setup.
- Reinstalling from a *different* URL starts you fresh — it won't see data
  saved under the old URL, so you'll go through setup again.
- There's no cloud sync between devices; each device keeps its own setup
  and data.
...
- **Export data** downloads a JSON file (`daily-tracker-backup-YYYY-MM-DD.json`)
  containing your tracker setup and every check-in, score, and journal
  entry. Save it anywhere — email it to yourself, drop it in cloud storage,
...
- **Import data** loads a previously exported file back in. You'll be asked
  ...
  outright. On a fresh install with no setup yet, importing a backup will
  also carry over your tracker setup, so you skip onboarding entirely.
...
that file on the new install to carry your setup and history over.
